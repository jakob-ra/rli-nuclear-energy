...
plt.savefig(os.path.join(path, 'Plots', 'number-of-articles-by-source.pdf'))
plt.show()
...
plt.savefig(os.path.join(path, 'Plots', 'number-of-articles-over-time.pdf'))
plt.show()
...
plt.savefig(os.path.join(path, 'Plots', 'articles-by-length.pdf'))
plt.show()

# Length of articles by source
df.groupby('source_agg').length.mean().sort_values().plot(kind='barh')
plt.ylabel('Source')
plt.xlabel('Average length in words')
plt.savefig(os.path.join(path, 'Plots', 'mean-article-length-by-source.pdf'))
plt.show()
...
stopwords += ['oorspronkelijke', 'pagina', 'staat', 'Staaten']
wordcloud = WordCloud(
...
plt.axis('off')
plt.savefig(os.path.join(path, 'Plots', 'word-cloud-overall.pdf'), bbox_inches='tight')
plt.show()